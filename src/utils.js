 ...
 * @param {AxiosRequestConfig} fetchOptions
 */
export function getCountry(fetchOptions) {
  return new Promise((resolve, reject) => {
    if (!fetchOptions.url) {
      return reject(new Error("URL not defined in `fetchOptions`"));
    }
    return axios
      .request(fetchOptions)
      .then((response) => {
        if (!response.data.iso2) {
          return reject(new Error("Country code not found in response"));
        }
        return resolve(response.data.iso2);
      })
...
export const allProps = [
  ...
  },
  {
    ...
    /**
     * @type {boolean | AxiosRequestConfig}
     */
    default: false,
    type: [Boolean, Object],
    description: "Fetch the country of the user from a custom URL",
    inDemo: false,
  },
  {
  ...
  },
  {
    ...
    default: "auto",
    type: String,
    description: `Allowed values: <code>'auto'</code> (Default set by phone),
  ...
  },
  {
    ...
    default: false,
    type: Boolean,
    description: `Only allow valid characters in a phone number
